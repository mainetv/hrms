@php
  // dump([
    // \DB::table('permissions')->pluck('name'),
      ...
      // 'view_as_role_id' => session('view_as_role_id'),
      // 'view_as_division_id' => session('view_as_division_id'),
      // 'view_as_label' => session('view_as_label'),
  // ]);
  $getRequestStatusSignatories = getSignatories(['module_id' => 5])->mapWithKeys(function($row) {
    $user = $row->user;
    if (!$user) return [];

    $middle = optional($user)->middle_name
        ? strtoupper(substr($user->middle_name, 0, 1)) . '.'
        : '';

    $fullName = trim("{$user->first_name} {$middle} {$user->last_name}");
    return [$user->id => strtoupper($fullName)];
  });

  $getPayees = getPayees();
  $getLibs = getLibs($project->id);
  $getRsTransactionTypes = getRsTransactionTypes();
@endphp
{{-- debug switch role --}}
<div style="background:#f5f5f5; padding:5px; margin-bottom:10px;">
<strong>label:</strong> {{ $viewAsLabel }} |
<strong>role_id:</strong> {{ $viewAsRoleId }} |
<strong>division_id:</strong> {{ $viewAsDivisionId }}
</div>
...
<form id="request-status-form">
  ...
  <div class="card">
    <div class="card-header">
      <div class="row">
        <h5 class="page-title">Edit Budget Utilization Request Status</h5>  
